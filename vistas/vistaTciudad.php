...
require_once('../controladores/corTciudad.php');
$objFunciones = new clsFunciones;
$operacion = $lcOperacion;
$listo = $lcListo;
if(($operacion!='buscar' && $listo!=1) || ($operacion!='buscar' && $listo==1))
{
	$id = $objFunciones->ultimo_id_plus1('tciudad','codigo');
}else{
	$cmunicipio = $objFunciones->combo_segun_combo("tmunicipio","codigo","nombre","codigo_estado",$estado,$municipio);
	$cparroquia = $objFunciones->combo_segun_combo("tparroquia","codigo","nombre","codigo_municipio",$municipio,$lcCodigo_parroquia);
}
?>
...
<td><select name='txtcodigo_parroquia' disabled='disabled' id='txtcodigo_parroquia' class='validate[required]'>
<option value=''>Seleccione</option>
...
<?php $objFunciones->botonera_general('Tciudad','total',$id); ?>
</div>
</form>
<!--
	Codigo
